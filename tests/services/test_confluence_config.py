...
def test_fetch_page_details_success(tmp_path):
    from unittest.mock import patch, MagicMock
    cfg = ConfluenceConfig(config_dir=tmp_path)
    cfg.cloud_domain = "test.atlassian.net"
    cfg.cloud_api_token = "tok"
    ...
    with patch("md2conf.api.ConfluenceAPI") as mock_api_class:
        ...
        details = cfg.fetch_page_details("12345")
        assert details == {
            "confluence_id": "12345",
            "confluence_space": "SPACE",
            "confluence_page_name": "Page Title",
            "confluence_url": "https://test.atlassian.net/wiki/spaces/SPACE/pages/12345",
        }
        mock_api_instance.get_page.assert_called_once_with("12345")
        mock_api_instance.space_id_to_key.assert_called_once_with("space-123")


def test_fetch_page_details_server_success(tmp_path):
    from unittest.mock import patch, MagicMock
    cfg = ConfluenceConfig(config_dir=tmp_path)
    cfg.deployment = DeploymentType.SERVER
    cfg.server_url = "https://km.fubonlife.com.tw"
    cfg.server_context_path = "/confluence"
    cfg.server_username = "user"
    cfg.server_password = "pwd"
    ...
    with patch("md2conf.api.ConfluenceAPI") as mock_api_class:
        ...
        mock_page.title = "Page Title"
        mock_api_instance.get_page.return_value = mock_page
        mock_api_instance.space_id_to_key.return_value = "IPK"
        mock_api_class.return_value.__enter__.return_value = mock_api_instance
        
        details = cfg.fetch_page_details("1897818658")
        assert details == {
            "confluence_id": "1897818658",
            "confluence_space": "IPK",
            "confluence_page_name": "Page Title",
            "confluence_url": "https://km.fubonlife.com.tw/confluence/spaces/IPK/pages/1897818658",
        }
        mock_api_instance.get_page.assert_called_once_with("1897818658")
        mock_api_instance.space_id_to_key.assert_called_once_with("space-123")
